...
function nameOf(file) { let name=file.replace(/\.gif$/i,'').replace(/[_-]+/g,' ').replace(/\s+/g,' ').trim(); for (const [from,to] of replacements) name=name.replace(from,to); return name.replace(/\s+/g,' ').replace(/\b\w/g,c=>c.toUpperCase()).trim(); }
