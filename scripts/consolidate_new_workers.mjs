...
const assigned = manifest.workers.flatMap((worker) => worker.mediaIds);
...
for (const worker of manifest.workers) {
  ...
  for (const item of results) {
    if (!worker.mediaIds.includes(item.mediaId)) throw new Error(`${worker.workerId} returned unassigned ${item.mediaId}`);
    const missing = required.filter((field) => empty(item[field]));
    if (!Array.isArray(item.instructions) || item.instructions.length < 3) missing.push('instructions<3');
    if (!['HIGH','MEDIUM','LOW'].includes(String(item.confidence).toUpperCase())) missing.push('confidence-value');
    if (missing.length) throw new Error(`${item.mediaId} failed quality gate: ${missing.join(',')}`);
    if (!catalogById.has(item.mediaId)) throw new Error(`Unknown media ${item.mediaId}`);
    received.push({ ...item, confidence: String(item.confidence).toUpperCase(), identificationStatus: 'curated', needsManualReview: Boolean(item.reviewRequired) });
  }
}
if (new Set(received.map((item) => item.mediaId)).size !== assigned.length) throw new Error('Duplicate or missing returned IDs');
...
manifest.status = 'consolidated';
manifest.consolidatedAt = new Date().toISOString();
manifest.consolidatedMediaIds = received.map((item) => item.mediaId);
